...
| `/api` | `routes/sales-agents.js` | **Agenci handlowi** (zakładka Agenci): CRUD agentów (**waluta rozliczeń EUR/PLN** per agent), **cennik agenta** (`/prices` — inna cena na każdy produkt, upsert po nazwie), przypinanie kontrahentów PL/ES (od `since`), operacje delivery/settlement/adjustment (waluta domyślnie = waluta agenta). **Saldo per waluta = Σ FV kontrahentów agenta − towar wydany − rozliczenia + korekty** (np. 100×4,50 FV − 100×3 towar = 150 €); karta agenta zwraca też scaloną historię (operacje + FV) |
