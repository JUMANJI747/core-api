...
| `/api` | `routes/sales-agents.js` | **Agenci handlowi** (zakładka Agenci): CRUD agentów, przypinanie kontrahentów PL/ES (od `since`), operacje delivery/settlement/adjustment. **Saldo per waluta = Σ FV kontrahentów agenta − towar wydany − rozliczenia + korekty** (np. 100×4,50 FV − 100×3 towar = 150 €); karta agenta zwraca też scaloną historię (operacje + FV) |
...
- **Agenci handlowi:** `SalesAgent`, `SalesAgentContractor` (przypięcie kontrahenta PL/ES do agenta, `since` = od kiedy FV liczą się do salda, jeden kontrahent → jeden agent), `SalesAgentOperation` (`delivery` towar wydany po cenie agenta / `settlement` rozliczenie np. gotówką / `adjustment` korekta ze znakiem).
