...
| `/api` | `routes/invoices.js` | **FV PL (iFirma)**: invoice-preview / invoice-confirm(-latest), pay, ksef-status, shipment-doc, last-price, link/unlink-shipment, **invoice-draft-from-email** (prefill formularza FV z maila), **parse-order** (rozbij wklejone zamówienie na pozycje+ceny, Sonnet), **fix-unknown-numbers** (nadrób numery FV zapisane jako UNKNOWN), cache zamówień GK |
