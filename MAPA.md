...
| `/api` | `routes/accounting.js` | **„Dodatkowa księgowość"**: monthly-report (odświeża statusy KSeF w tle przez `ksef-sales-sync`, **throttle 10 min** — raport wczytuje się też po KAŻDEJ akcji, więc bez tego padał limit 20/h), refresh-ksef-status (guzik „Odswiez status KSeF” — WYMUSZONY odczyt dla ogladanego miesiaca, omija throttle, respektuje karencje po 429), send-month-to-ksef (masowa wysylka + JEDEN odczyt numerow po ~90 s), pair-wdt, pair-wdt-one |
