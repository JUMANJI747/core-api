...
- **Mail → Telegram:** `inbox-poller` → klasyfikacja → web-order/attachment-order/standard → Telegram + web push. Powiadomienie o zamówieniu zapisuje pozycje+kontrahenta do `AgentContext 'order-notification'` — odpowiedź „wystaw fv na to zamówienie" dostaje je wstrzyknięte w `accounting-agent` (push żyje poza rozmową agenta).
