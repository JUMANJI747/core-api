...
- **Agenci handlowi:** `SalesAgent`, `SalesAgentContractor` (przypięcie kontrahenta PL/ES do agenta, `since` = od kiedy FV liczą się do salda, jeden kontrahent → jeden agent), `SalesAgentPrice` (cennik agenta: nazwa produktu + jego cena, unikalna per agent), `SalesAgentOperation` (`delivery` towar wydany po cenie agenta / `settlement` rozliczenie np. gotówką / `adjustment` korekta ze znakiem).
