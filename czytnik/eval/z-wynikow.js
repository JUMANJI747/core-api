  ...
  for (const k of j.karty || []) {
    ...
    fs.writeFileSync(path.join(CEL, nazwa), JSON.stringify({
      ...
      okres: { rok: k.rok, miesiac: k.miesiac,
        nazwiska: JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'dane', 'pracownicy.json'), 'utf8')).lista,
        stawkiDnia: JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'dane', 'pracownicy.json'), 'utf8')).stawkiDnia || {},
