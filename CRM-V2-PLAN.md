...
aliases             String[]  @default([])    // alternatywne nazwy + potoczne ("ACME Sp. z o.o.", "ACME", "ACME Polska", "Janusz", "ten od Krakowa")
...
**Auto-population aliases (agent-side):** gdy agent (Master / mail-classifier / telegram-handler) widzi w kontekście rozmowy nową nazwę dla kontrahenta zidentyfikowanego po innym sygnale (NIP / email / Telegram chatId) i ta nazwa nie znajduje się w `aliases`, dorzuca ją:
- po fuzzy match >0.85 vs istniejące aliases / `name` → ignore (już mamy wariant),
- po sanity check (długość 2-80 znaków, nie zawiera adresu URL, nie zawiera maila) → `prisma.contractor.update({ where:{id}, data:{ aliases: { push: newAlias } } })` + emit `contractor.alias_added` z `payload: { alias, source: 'agent' | 'mail' | 'telegram' }`.

To rozwiązuje problem "user pisze 'wyślij paczkę Januszowi' a w bazie kontrahent ma nazwę 'Janusz Nowak Sp. z o.o.' — agent przy okazji dorzuca 'Janusz' do aliases, następnym razem znajdzie szybciej."
...
- `agent.recent_activity_pulled` / `sync.imap.poll*` — **30 dni** (debug, nie biznes),
- `agent.run_*` — **90 dni** (run zawiera całą rozmowę + tool calls + odpowiedź, długie payloady ale wartość audytowa wysoka),
- `telegram.*` — **90 dni**,
...
#### Agent (4) — run-level logging, nie per-tool-call

**Filozofia (zaktualizowana po dyskusji):** jeden `agent.run` = jeden ciąg "user query → tool calls → final answer". Nie logujemy każdego tool calla osobno (zalewałoby timeline), zamiast tego cały run zapisany w jednym evencie z pełną historią.

| Plik / punkt | Typ |
|---|---|
| `agent-runtime.js` start runu (user submit) | `agent.run_started` (z `payload: { agent, userQuery, runId }`) |
| `agent-runtime.js` koniec runu (final response) | `agent.run_finished` (z `payload: { runId, userQuery, toolCalls: [{tool, input, output}, ...], finalText, durationMs, totalTokens }`, truncate per polityka 10KB — jak run gigantyczny, zostaje meta + `extras.truncated=true`) |
| `agent-runtime.js` exception | `agent.run_failed` (z `payload: { runId, userQuery, toolCallsBefore, error }`) |
...
**Konsekwencje:**
- Timeline pokazuje "user spytał X → agent zrobił Y, Z, W → odpowiedział tak", jeden wpis.
- Filter `agent.run_failed` daje od razu listę run-ów do diagnozy.
- Tool calls historycznie dostępne w `payload.toolCalls`, można pełnotekstowo szukać po `payload->toolCalls`.
- `runId` jest też reference-em z evenctów spowodowanych runem (np. `mail.sent` z `extras.agentRunId`) → join z `agent.run_finished` daje pełen kontekst akcji.
...
**Etap 5a (do uruchomienia w tygodniu CRM v2):** NocoDB jako Railway service self-hosted nad tym samym Postgres. **Pełne uprawnienia CRUD** (nie read-only) na:
- `Contractor`, `ContractorContact`, `ContractorAddress` — edytujesz dane kontaktowe gdy agent wpisze coś źle, korygujesz ręcznie zamiast tłumaczyć agentowi co zmienić,
- `Contractor.aliases` (nowe pole `String[]`) — lista potocznych nazw / skrótów którymi nazywasz klientów (`"Janusz"`, `"ten od Krakowa"`, `"Polonia Med"`). Agent przy spotkaniu nowego aliasu w mailu/Telegramie auto-appenduje go do listy. Ty możesz w NocoDB edytować ręcznie.
- `Invoice`, `Product`, `CnContractor`, `CnProduct` — korekty manualne (np. fix typo w nazwie),
- `Deal` (jeśli przeżyje rebuild front-u).

**Read-only viewy** do: `ActivityEvent` (timeline append-only, nie edytujemy historii), `Transaction`, `Email`, `Mailing`, `EmailLog`.

**Bezpieczeństwo:** NocoDB ma własny login (basic auth + 2FA jak chce), nie wystawiamy publicznie — dostęp tylko z Twojego IP / VPN-a lub na `https://nocodb.<railway-private-domain>` z auth na poziomie Railway. Co edytujesz w NocoDB nie tworzy `ActivityEvent` automatycznie — jeśli chcesz audit trail manualnych zmian, dodamy w v2.1 trigger Prisma.
...
17. `refactor(prisma): rename Es* → Cn* (Canarias)` — `EsContractor→CnContractor`, `EsInvoice→CnInvoice`, `EsProduct→CnProduct`, `EsAddress→CnAddress`, `EsInvoiceLineItem→CnInvoiceLineItem`, pole `Contractor.linkedEsContractorId → linkedCnContractorId`. Schema rename + update wszystkich referencji w `src/`, `agents/`, prompty Master/Subagent. Migracja Postgres: `ALTER TABLE ... RENAME TO ...` (data nietknięta). Eventy w słowniku: `es_invoice.* → cn_invoice.*` z migration job dla istniejących `ActivityEvent.eventType`. **Odpalamy po Etapie 4** żeby nie hamować rozwoju activity log.

Każdy commit z testem ręcznym (curl + opis w body PR-a) i logiem Railway.

**Uwaga semantyczna:** dotychczasowe `Es*` tabele to **tylko Kanary** (IGIC 0/3/7/9.5/15/20, Contasimple), **nie Hiszpania kontynentalna** (VAT 4/10/21). Jeśli kiedyś dojdzie Hiszpania mainland, dorobimy `EsContractor`/`EsInvoice` jako osobne tabele — nie mieszamy z Canarias bo inne VAT regime i prawdopodobnie inny system księgowy.
...
## Decyzje (zamknięte pytania, dla audytu)

1. ✅ **InvoiceLineItem dla CnInvoice (dawniej EsInvoice) — osobne tabele.** `InvoiceLineItem` (PL VAT 0/5/8/23) + `CnInvoiceLineItem` (IGIC 0/3/7/9.5/15/20).
2. ✅ **Backfill InvoiceLineItem niekompletnych FV — iFirma API NIE dostarcza pozycji** (tylko summary, pozycje są tylko w PDF). Decyzja: dla FV od 2026-01-01 spróbujemy PDF parsing w osobnym etapie, dla pre-2026 zostawiamy `extras.incomplete=true` i nie kombinujemy.
3. ✅ **ActivityEvent.payload — truncate >10KB** + zapis `extras.truncated=true` + `extras.originalSize`. DB 5-10GB jest komfortowy budżet.
4. ✅ **NocoDB self-hosted + WRITE access** (nie read-only). User wpisuje ręczne korekty bezpośrednio w bazie, łatwiej niż tłumaczyć agentowi.
5. ✅ **Stare pola Contractor — quick-access denorm.** Sync z `ContractorAddress(isPrimary=true)` przy update.
6. ✅ **Stary `Activity` (per-Deal) — nietknięty, legacy.** Sync z Google Sheets nigdy nie zadziałał, zostawiamy zamrożone. Nowa `ActivityEvent` to clean start. Front zdecyduje czy kiedyś scalimy.
7. ✅ **Tagi — hybryda.** Kanoniczne prefixy (`country:`, `lang:`, `carrier:`, `inbox:`) auto-generowane, free-form (`urgent`, `complaint`, `follow_up`) walidowane delikatnie (lowercase normalize, dedup).
8. ✅ **Cron trigger — C (external).** cron-job.org / GH Actions → POST `/api/cron/<job>?key=...`. Klucz w env.
9. ✅ **Agent log — RUN-LEVEL** (nie tool-level). Jeden `agent.run_finished` event = pełen ciąg user query → tool calls → final answer w payloadzie. `agent.tool_call` skreślone ze słownika. Retention 90 dni.
10. ✅ **`telegram.in` — pointer-only.** `eventType=telegram.in` z `payload={chatId, text:snippet}`, pełna treść w istniejących tabelach `Chat`/`Message`.
11. ✅ **Es* → Cn* rename** (Canarias-specific). Dedykowany commit #17 po Etapie 4. Reminder: Es* tabele to nie Hiszpania, to Kanary (IGIC).
12. ✅ **Start: Etap 1 — Contractor normalization.** Pierwszy commit = `aliases`/`primaryEmail`/`externalIds`/`linkedCnContractorId` + backfill z `extras`.
13. ✅ **Tracking — mail-driven primary + batch poll 2×/dzień fallback.** Sekcja 6.6.

## Otwarte pytania (do dopytania po starcie)

- Po Etapie 4: które front-endowe widoki najpierw (Customer 360 / Timeline / Dashboard / Editor)?
- Po Etapie 6: czy podpinamy NocoDB pod Railway internal domain z basic auth, czy chcesz VPN-only?
- Dla agent runów — czy chcesz w `agent.run_finished` payloadzie zapisywać też **prompt template** użyty przez subagenta (czyli wersję promptu), żeby móc historycznie zobaczyć "agent działał na promptcie v5 a teraz v7"? (Nice-to-have do A/B promptów.)
- Po Etapie 14 (cron): czy chcesz dzienne podsumowanie sync-ów na Telegram (`📊 ifirma: 12 nowych FV, contasimple: 3, gk: 0 problemów`) o 09:00, czy alerty tylko przy `*.failed`?

Po Twoim OK ruszamy od commitu 1.
