...
router.post('/sync-customers', asyncHandler(async (req, res) => {
  ...
  for (const c of list) {
    ...
    if (existing) {
      ...
      // Auto-przypisanie owner przy CREATE — Fuerteventura (ZIP 356xx / miasto z
      // listy / province) -> rogacz, reszta -> nikodem. Przy UPDATE nie tykamy
      // owner zeby nie nadpisac recznych zmian; rebackfill robi sie przez
      // POST /contractors/backfill-owners gdy user chce.
      const ownerAuto = resolveOwnerFromAddress({ postalCode: c.postalCode, city: c.city, province: c.province });
      saved = await prisma.esContractor.create({ data: { ...data, owner: ownerAuto } });
      created++;
    }
    ...
    select: {
      ...
      ...(wantLines ? {
        lineItems: {
          ...
          select: {
            position: true, ean: true, name: true, unit: true, qty: true,
            unitPriceNetto: true, vatRate: true, vatAmount: true,
            totalNetto: true, totalGross: true, currency: true,
          },
        },
      } : {}),
    },
...
// POST /api/contasimple/contractors/backfill-owners
// Body: { force?: boolean, dryRun?: boolean }
// Aplikuje regule adresowa (Fuerteventura ZIP 356xx / miasto z listy ->
// rogacz, reszta -> nikodem) jednorazowo. Domyslnie: tylko gdzie owner IS
// NULL (zachowuje reczne przypisania). force=true: nadpisuje wszystko.
// dryRun=true: nie zapisuje, zwraca tylko liczby + sample.
router.post('/contractors/backfill-owners', asyncHandler(async (req, res) => {
  const { force = false, dryRun = false } = req.body || {};
  const where = force ? {} : { owner: null };
  const candidates = await prisma.esContractor.findMany({
    where,
    select: { id: true, owner: true, city: true, province: true, postalCode: true, name: true, nif: true },
  });
  const planned = candidates.map(c => ({
    id: c.id, name: c.name, nif: c.nif,
    currentOwner: c.owner,
    newOwner: resolveOwnerFromAddress({ postalCode: c.postalCode, city: c.city, province: c.province }),
    city: c.city, postalCode: c.postalCode,
  }));
  const counts = {
    rogacz: planned.filter(p => p.newOwner === 'rogacz').length,
    nikodem: planned.filter(p => p.newOwner === 'nikodem').length,
  };
  if (dryRun) {
    return res.json({
      ok: true,
  ...
  const rogIds = planned.filter(p => p.newOwner === 'rogacz').map(p => p.id);
  const nikIds = planned.filter(p => p.newOwner === 'nikodem').map(p => p.id);
  const [rogRes, nikRes] = await Promise.all([
    rogIds.length
      ? prisma.esContractor.updateMany({ where: { id: { in: rogIds } }, data: { owner: 'rogacz' } })
      : Promise.resolve({ count: 0 }),
    nikIds.length
      ? prisma.esContractor.updateMany({ where: { id: { in: nikIds } }, data: { owner: 'nikodem' } })
      : Promise.resolve({ count: 0 }),
  ]);
  res.json({
    ok: true,
    scanned: planned.length,
    assigned: { rogacz: rogRes.count, nikodem: nikRes.count },
  });
}));
