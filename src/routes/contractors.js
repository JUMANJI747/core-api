...
// Cleanup duplikatow Email z messageId=NULL. Grupuje po
// (fromEmail|subject|createdAt rounded to minute) — zostawia
// najstarszy rekord per grupa, usuwa reszta. Body: { dryRun?: boolean }
router.post('/admin-dedupe-null-emails', async (req, res) => {
  const prisma = req.app.locals.prisma;
  const dryRun = !!req.body.dryRun;
  try {
    const emails = await prisma.email.findMany({
      where: { messageId: null },
      select: { id: true, fromEmail: true, subject: true, createdAt: true },
      orderBy: { createdAt: 'asc' },
    });
    const groups = new Map();
    for (const e of emails) {
      const key = `${(e.fromEmail || '').toLowerCase()}|${e.subject || ''}|${Math.floor(new Date(e.createdAt).getTime() / 60000)}`;
    ...
    for (const [, recs] of groups) {
      if (recs.length > 1) {
        groupsWithDups += 1;
        toDelete.push(...recs.slice(1).map(r => r.id));
      }
    }
    const stats = {
      totalNullMessageId: emails.length,
      uniqueGroups: groups.size,
      groupsWithDups,
      duplicatesToDelete: toDelete.length,
    };
    if (!dryRun && toDelete.length) {
      const result = await prisma.email.deleteMany({ where: { id: { in: toDelete } } });
      stats.actuallyDeleted = result.count;
    }
    res.json({ ok: true, dryRun, ...stats });
  } catch (e) {
    res.status(500).json({ error: e.message });
